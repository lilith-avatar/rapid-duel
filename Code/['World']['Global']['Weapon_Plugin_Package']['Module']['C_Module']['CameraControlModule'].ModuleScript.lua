...
function CameraControl:Init()
	...
	self.FixUpdateTable = {}
	--蹲高度变化
	self.crouchController = TweenController:new(
		'crouch',
		self,
		function() return 0.4 end,
	...
	---挨打镜头抖动
	self.ShakeController = TweenController:new(
		'shake',
		self,
		function() return self.shakeTime end,
		function(_t1, _t2)
			math.randomseed(Timer.GetTime() * 10000)
			self.deltaOffset = Vector3(Shake(self.shakeStrenth), Shake(self.shakeStrenth), Shake(self.shakeStrenth)) * _t1 / _t2
		end,
		function() self.deltaOffset = Vector3.Zero end,
		true
	)
end
