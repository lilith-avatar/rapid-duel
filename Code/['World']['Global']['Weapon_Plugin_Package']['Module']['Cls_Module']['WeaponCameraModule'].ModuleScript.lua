...
function WeaponCamera:initialize(_gunRecoil)
	...
	---尝试使用协程控制器
	--z轴旋转动画
	self.selfSpinController = TweenController:new(
		'selfSpin',
		self,
		function()
		...
		function(_time, _totalTime)
			self.m_gamma = self.selfSpinController.Ampl * math.exp(-self.config_vibrationDump * _time) * math.sin(self.config_vibrationOmega * _time + self.selfSpinController.phase)
		end,
		function()
			self.m_gamma = 0
		end,
		...
		function()
			...
			if(not remn) then
				...
				local tempA = self.selfSpinController.Ampl * math.exp(-self.config_vibrationDump * self.selfSpinController.time)
				local tempO = tempA * self.config_vibrationOmega * math.cos(self.config_vibrationOmega * self.selfSpinController.time + self.selfSpinController.phase)
	...
	---FOV跳动总动画
	self.jumpFOVController = TweenController:new(
		'jumpFOV',
		self,
		function()
	...
	---枪口跳动总动画
	self.jumpController = TweenController:new(
		'jump',
		self,
		function() return self.config_jumpTime end,
		function(_t1, _t2, _dt)
	...
	---枪口回复总动画
	self.recoverController = TweenController:new(
		'recover',
		self,
		function() return self.m_backTime end,
		function(_t1, _t2, _dt)
		...
		function() end,
		true,
		function() self.recoverController.total = 0 end
	)
	
	---辅瞄动画
	self.assistAimController = TweenController:new(
		'assistAim',
		self,
		function() return self.gun.config_assistAimTime end,
		function(_t1, _t2, _dt)
		...
		function()
			...
			local thetaTotal = math.atan(relativePos.y / Vector2(relativePos.x, relativePos.z).Magnitude) - (90 - Vector3.Angle(world.CurrentCamera.Forward, Vector3.Up)) / 180 * math.pi
			local phyTotal = Vector2.Angle(Vector2(relativePos.x, relativePos.z), Vector2(self.m_camera.Forward.x, self.m_camera.Forward.z)) 
				* math.pi / 180
	...
	self.aimController = TweenController:new(
		'aim',
		self,
		function() return self:GetAimTime() end,
		function(_t1, _t2, _dt)
			local por = _t1 / _t2
	...
	--关镜总动画
	self.deaimController = TweenController:new(
		'deaim',
		self,
		function() return self.gun.config_stopAimTime end,
		function(_t1, _t2, _dt)
			local por = _t1 / _t2
...
function WeaponCamera:GetEnemies()
	...
	for i, v in pairs(FindAllPlayers()) do
		if v ~= self.gun.character and v.PlayerType and
				v.PlayerType.Value ~= self.gun.character.PlayerType.Value and
				v.Health > 0  and v.Avatar.Bone_Head.HeadPoint then
			table.insert(res, v)
		end
	end
	return res
...
function WeaponCamera:IsUp(_pos)
	...
	return math.atan(relativePos.y / Vector2(relativePos.x, relativePos.z).Magnitude) > (90 - Vector3.Angle(world.CurrentCamera.Forward, Vector3.Up) * math.pi / 180)
end
