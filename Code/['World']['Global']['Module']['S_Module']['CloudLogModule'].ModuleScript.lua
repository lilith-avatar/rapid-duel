...
function CloudLog:PlayerRemove(_player)
    ...
    if _player.PlayerState.Value == Const.PlayerStateEnum.OnHall_NoMatching
    or _player.PlayerState.Value == Const.PlayerStateEnum.OnHall_Matching then
        ---埋点需求002..情况2
...
function CloudLog:PlayerDieEventHandler(_killer, _killed, _weaponId, _hitPart)
    local mes4 = tostring(curSceneId)..'/'..tostring(_killer.Position)..'/'..tostring(_killed.Position)..'/'..tostring(_weaponId)..'/'..tostring(_hitPart)
